...
export class GoPgComponent implements OnInit {
  ...
  ngOnInit(): void {
   this.Nb= this.localStorageService.getItem("_Words");
  }

  getWords(qa: number){
    for (let j: number=1; j <= qa; j++){
      this.randomInteger(0,this.localStorageService.getLenth()-4);
      this.words = this.localStorageService.getKey(this.i);      
  ...
  checkWords(qa: number){
    for (let j: number=1; j <= qa; j++){      
      if (this.localStorageService.getItem(document.getElementById(`${j}`).innerText) == document.querySelectorAll("input")[j-1].value){
